...
function ExtendedBrush (type) {
  if (arguments.length === 0) { go.Brush.call(this) } else { go.Brush.call(this, type) }
}
...
ExtendedBrush._angleBetweenSpots = function (brush) {
  ...
  var end = brush.end
  if (isNaN(start.x + start.y + end.x + end.y)) { throw new Error('The brush does not have valid spots') }
...
ExtendedBrush._englishPositionToCoordinate = function (str) {
  ...
  if (str.indexOf('bottom') > -1) { y = 1 } else if (str.indexOf('top') > -1) { y = 0 }
  if (str.indexOf('left') > -1) { x = 0 } else if (str.indexOf('right') > -1) { x = 1 }
  ...
  if (split.length === 1) {
    ...
    if (digits === null) {
      ...
      if (split[0].indexOf('circle') > -1 && split[1].indexOf('%') < 0) {
        ...
        return [r, r]
      } else { throw new Error('Invalid CSS shape description') }
    ...
    if (split[0] !== 'ellipse') { throw new Error('invalid CSS shape description') }
...
ExtendedBrush._parseExtentKeyword = function (str, w, h, center) {
  ...
  if (split === null) { throw new Error('invalid extent keyword') } else { extent = split[split.length - 1] }
...
ExtendedBrush.CSSStringToRGB = function (CSSColorString) {
  if (!go.Brush.isValidColor(CSSColorString)) { throw new Error('Invalid CSS Color String: ' + CSSColorString) }
...
ExtendedBrush._RGB_XYZ_Inverse_Companding = function (RGB_value) {
  ...
  return Math.pow(((RGB_value + 0.055) / 1.055), 2.4)
}

ExtendedBrush._XYZ_RGB_Companding = function (XYZ_value) {
  if (XYZ_value * 12.92 <= 0.04045) { return XYZ_value * 12.92 }
  return 1.055 * Math.pow(XYZ_value, 0.416667) - 0.055
}
...
ExtendedBrush._RGBtoXYZ = function (rgb) {
  ...
  var applied = []
  var i
  for (i = 0; i < rgb.length; i++) { rgb[i] = ExtendedBrush._RGB_XYZ_Inverse_Companding(rgb[i]) }
  for (i = 0; i < rgb.length; i++) { applied[i] = ExtendedBrush._rowMultiplication(ExtendedBrush._sRGBtoXYZMatrix[i], rgb) }
  return applied
}
...
ExtendedBrush._XYZtoRGB = function (xyz) {
  var applied = []
  var i
  for (i = 0; i < xyz.length; i++) { applied[i] = ExtendedBrush._rowMultiplication(ExtendedBrush._XYZtosRGBMatrix[i], xyz) }
...
ExtendedBrush._Lab_XYZ_HelperFunction = function (inputLab) {
  ...
  if (inputCbd > ExtendedBrush._Lab_EPSILON) { return inputCbd }
