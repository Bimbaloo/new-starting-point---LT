  ...
  for (var y = yBegin; y < yEnd; ++y) {
    for (var x = xBegin; x < xEnd; ++x) {
      ...
      for (var a = 0; a < 6; ++a) {
        ...
        vertices.push(xm * scale + m, ym * scale + m, 0.0)
      }
    }
  }
...
function createSuperShape (shader, params) {
  ...
  for (var i = 0; i < 3; ++i) {
    baseColor[i] = (random.uInt() % 155 + 100) / 255.0
  }
...
function configureLightAndMaterial () {
  var light0Direction = modelview.transform3([-4.0, 1.0, 1.0])
  var light1Direction = modelview.transform3([1.0, -2.0, -1.0])
  var light2Direction = modelview.transform3([-1.0, 0.0, -4.0])
...
function drawModels (zScale) {
  ...
  for (var x = -2; x <= 2; ++x) {
    ...
    modelview.push()
    modelview.translate(-4.0, offs, 4.0)
    modelview.rotate(-90.0, 0.0, 0.0, 1.0)
    ship.draw()
    modelview.pop()
  }
}
...
Matrix4x4.prototype.lookAt = function (eyeX, eyeY, eyeZ,
  ...
  lookAt.elements[3 * 4 + 0] = 0.0
  lookAt.elements[0 * 4 + 1] = y[0]
  lookAt.elements[1 * 4 + 1] = y[1]
  lookAt.elements[2 * 4 + 1] = y[2]
  lookAt.elements[3 * 4 + 1] = 0.0
  lookAt.elements[0 * 4 + 2] = z[0]
  lookAt.elements[1 * 4 + 2] = z[1]
  lookAt.elements[2 * 4 + 2] = z[2]
  lookAt.elements[3 * 4 + 2] = 0.0
  lookAt.elements[0 * 4 + 3] = 0.0
  lookAt.elements[1 * 4 + 3] = 0.0
  lookAt.elements[2 * 4 + 3] = 0.0
  lookAt.elements[3 * 4 + 3] = 1.0
...
function appInit () {
  ...
  litShader.bind()
  gl.uniform4f(litShader.ambientLoc, 0.2, 0.2, 0.2, 1.0)
  gl.uniform4f(litShader.light0DiffuseLoc, 1.0, 0.4, 0, 1.0)
  gl.uniform4f(litShader.light1DiffuseLoc, 0.07, 0.14, 0.35, 1.0)
  gl.uniform4f(litShader.light2DiffuseLoc, 0.07, 0.17, 0.14, 1.0)
  gl.uniform4f(litShader.light0SpecularLoc, 1.0, 1.0, 1.0, 1.0)
  gl.uniform1f(litShader.shininessLoc, 60.0)
