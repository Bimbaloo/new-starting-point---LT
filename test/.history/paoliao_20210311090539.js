 ...
 * @Description: 
 * @LastEditors: 马燥
 * @LastEditTime: 2021-03-11 09:05:39
 */
var app = new Vue({
  el: "#app",
  data: {
  ...
  created: function () {
    this.$nextTick(() => {
      Object.keys(this.lableWidth.paoliao).forEach(el => {
        const strLength = document.querySelector(`.${el} .el-form-item__label`).textContent.length * 10
        if (strLength > this.lableWidth.paoliao[`${el}`]) {
          this.lableWidth.paoliao[`${el}`] = strLength
        }
      })
    })
  },
